...
type Heuristics m axiom touchable types constraint ci diagnostic = Path m axiom touchable types constraint ci -> [Heuristic m axiom touchable types constraint ci diagnostic]

-- | A heuristic data type that is either a voting heuristic of a filter heuristics
data Heuristic m axiom touchable types constraint ci diagnostic
    = Filter String 
    ...
    | Voting [VotingHeuristic m axiom touchable types constraint ci diagnostic]
    
-- | A voting heuristic that can either be used on a single part of the path or on the entire path at once
data VotingHeuristic m axiom touchable types constraint ci diagnostic
    = SingleVoting String (HasTypeGraph m axiom touchable types constraint ci diagnostic => (constraint, EdgeId, ci, GraphModifier m axiom touchable types constraint ci) -> m (Maybe (Int, String, constraint, EdgeId, ci, GraphModifier m axiom touchable types constraint ci))) 
    | MultiVoting String (HasTypeGraph m axiom touchable types constraint ci diagnostic => [(constraint, EdgeId, ci, GraphModifier m axiom touchable types constraint ci)] -> m (Maybe (Int, String, [constraint], [EdgeId], ci, GraphModifier m axiom touchable types constraint ci))) 
...
getVotingHeuristicName :: VotingHeuristic m axiom touchable types constraint ci diagnostic -> String
getVotingHeuristicName (SingleVoting s _) = s
getVotingHeuristicName (MultiVoting s _) = s

-- | Show instance for the heuristic
instance Show (Heuristic m axiom touchable types constraint ci diagnostic) where
...
instance Show (VotingHeuristic m axiom touchable types constraint ci diagnostic) where
...
applyHeuristics :: HasTypeGraph m axiom touchable types constraint ci diagnostic
                        => Heuristics m axiom touchable types constraint ci diagnostic
...
applyGraphModifier  :: HasTypeGraph m axiom touchable types constraint ci diagnostic
...
evalHeuristics   :: HasTypeGraph m axiom touchable types constraint ci diagnostic
                => [Heuristic m axiom touchable types constraint ci diagnostic] 
                -> Path m axiom touchable types constraint ci -- ^Edges that are allowed to be modified, might nog be continious
                -> TGGraph touchable types constraint ci 
...
evalVoteHeuristic   :: HasTypeGraph m axiom touchable types constraint ci diagnostic
                    => Path m axiom touchable types constraint ci
                    -> VotingHeuristic m axiom touchable types constraint ci diagnostic
...
highParticipation :: (HasLogInfo m, HasTypeGraph m axiom touchable types constraint ci diagnostic) => Double -> Path m axiom touchable types constraint ci -> Heuristic m axiom touchable types constraint ci diagnostic
highParticipation ratio _path =
    Filter "Participation ratio" selectTheBest
        where
            selectTheBest :: HasTypeGraph m axiom touchable types constraint ci diagnostic => [(constraint, EdgeId, ci, GraphModifier m axiom touchable types constraint ci)] -> m [(constraint, EdgeId, ci, GraphModifier m axiom touchable types constraint ci)]
            selectTheBest es = do
                graph <- getGraph
                axs <- getAxioms
